...
{
#ifdef _UNICODE
  WideCharToMultiByte(CP_UTF8, 0, unicode, -1, ascii, outSize, "?", NULL);
#else
...
void CDevIMI::IMIWaypoint(const Declaration &decl, unsigned imiIdx, TWaypoint &imiWp)
{
  ...
  const Declaration::TurnPoint &tp = decl.get_turnpoint(idx);
  ...
  if(imiIdx == 1) {
    ...
    imiWp.oz.R1 = (DWORD)std::min(250000u, tp.radius);
